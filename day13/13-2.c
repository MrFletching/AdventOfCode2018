...
void tick_world(World *world) {
    ...
    for(int c = 0; c < world->carts_count; c++) {
        ...
        for(int j = 0; j < world->carts_count; j++) {
            ...
            if(other_cart != cart && !other_cart->crashed && cart->x == other_cart->x && cart->y == other_cart->y) {
                cart->crashed = true;
                other_cart->crashed = true;
                world->crashed_carts_count += 2;
            }
        }
...
int main() {
    ...
    while(world.crashed_carts_count != world.carts_count - 1) {
        tick++;
        tick_world(&world);
        printf("Tick: %d\n", tick);
        //print_world(&world);
